...
function ComputeWScen(CVTS::Dict{String,Array{Tuple{Float64,Scenario}}},
    ...
    @objective(m,Min,
               ...
               +0.1*sum(lambda[i] for i=keysWS)/length(keysWS)
               )
    optimize!(m)
    return Scenario(merge(Dict{String,Float64}(i=> value(wscen[(i,"p")])
                                               for i=pertLoads),
...
function ComputeCVTS(CriticalScen::Array{Scenario},
                     ...
                     )::Dict{String,Array{Tuple{Float64,Scenario}}}
    alpha=DefAlpha(pertLoads,pert)
    CVTS=Dict{String,Array{Tuple{Float64,Scenario}}}()
    for i=keys(netgen)
...
function GradPush(Scen::Scenario,
                  CVTS::Dict{String,Array{Tuple{Float64,Scenario}}},PG0,V0,
                  ExploredConstr::Set{String},pertLoads::Set{String},
                  pert::Float64)
    (pertPUp,pertPDown,pertQUp,pertQDown) = BoundsLoads(pertLoads,pert)
    alpha=DefAlpha(pertLoads,pert)
    for i=keys(netgen)
    ...
    for c=ConstrViol
        ...
        for i=pertLoads
            if abs(WS.loadP[i]) > tol && PushScen.loadP[i]==Scen.loadP[i]#tol
                nbPushP+=1
            ...
            if abs(WS.loadQ[i]) > tol && PushScen.loadQ[i]==Scen.loadQ[i]#tol
                nbPushQ+=1
